...
class DocumentGeneratorApp:
        ...
        try:
            ...
            if 'IGST_SUM' not in row_data and all(f in normalized_row and pd.notna(normalized_row[f]) for f in
                ...
                row_data['IGST_SUM'] = sum([
                    ...
                    float(normalized_row.get('CGST_AS_IGST', 0)),
    ...
    def format_value(self, value, key=None):
        ...
        if key and 'gstin' in key.lower() and isinstance(value, str) and len(value) == 15:
            return f"{value[0:2]} {value[2:7]} {value[7:12]} {value[12:15]}"
    ...
    def generate_document(self, doc_type, row_data, idx):
        ...
        try:
            ...
            output_filename = f"{doc_type.replace(' ', '_')}_{invoice_num}_{timestamp}.docx"
    ...
    def replace_all_placeholders(self, doc, replacements):
        """Replace all placeholders in the document with more comprehensive handling"""
        try:
            used_placeholders = set()

            # First normalize replacement keys to handle variations
            ...
            for paragraph in doc.paragraphs:
                self._process_paragraph(paragraph, normalized_replacements, used_placeholders)
            ...
            # Log any unused placeholders that were provided but not found
            unused = set(normalized_replacements.keys()) - used_placeholders
            if unused:
                logging.warning(f"Unused placeholders in template: {unused}")
    ...
    def _process_paragraph(self, paragraph, replacements, used_placeholders):
        """Helper method to process a single paragraph"""
        ...
        for placeholder, value in replacements.items():
            for fmt in [f"{{{{{placeholder}}}}}", f"{{[{placeholder}]}}"]:
                if fmt in new_text:
                    new_text = new_text.replace(fmt, str(value))
